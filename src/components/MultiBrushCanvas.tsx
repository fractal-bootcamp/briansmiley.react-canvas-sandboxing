...
export interface CalligraphyCanvasProps {
  activeColor: string;
...
const CalligraphyCanvas = (props: CalligraphyCanvasProps) => {
  ...
  const sketch = (s: p5) => {
    ...
    s.draw = () => {
      ...
      //writebackgrounfd to get patterns, s.clear for transparency for samples
      writeBackground(s);
      //   s.clear();
        ...
        case "streak":
          ...
          if (s.mouseIsPressed && inStroke) {
            ...
            for (let i = 0; i < streak.splitNum; ++i) {
              ...
              streak.oldR +=
                (streak.brushRadius - streak.oldR) / streak.splitNum;
    ...
    s.mousePressed = () => {
      if (mouseOffCanvas() || s.mouseButton !== s.LEFT) return;
      onStrokeStart();
    };
    s.touchStarted = () => {
    ...
    function taperLine(
      ...
      const distance = s.sqrt(deltaX ** 2 + deltaY ** 2);
      const slope = s.atan2(deltaY, deltaX);
  ...
  return (
    <>
      <div
        className="flex justify-center w-fit border border-black"
        ref={canvasDivRef}
      ></div>
    </>
  );
